...
full_dat <- cbind( death = X$death, cbind(X_static), X_dynamic ) %>% na.omit
...
run_dlm <- function( x, nsamp = 50000, nburn = 50000, thin = 10 ) {
  DLM_MCMC_nb( death ~ ., data = full_dat,# groups = group_labs,
...
toc() # 658.103 sec elapsed

### MLE fit for comparison
mle_fit <- glm.nb( death ~ ., data = full_dat )

### Write results to file
all_res <- list( MCMC = mcmc_fit, MLE = mle_fit )
...
pdf( 'Chicago_Gamma_NB.pdf', width = 12 )
...
pdf( 'Chicago_DLF_NB.pdf', width = 12 )
DLF_plot( res )
dev.off()
