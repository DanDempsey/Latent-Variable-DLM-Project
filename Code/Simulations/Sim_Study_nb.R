...
for ( i in 1:outlen ) {
  ...
  lin_pred <- exp( -(covars_expanded %*% true_bet) )
  prob <- 1 / ( 1 + lin_pred )
  y <- rnbinom( N, true_xi, 1 - prob ) # 1 minus prob because it is parameterised by failure rate
  ...
  sim_res[[i]]$MLE <- glm.nb( y ~ ., data = full_dat )
